...
import { 
  ...
  FiCalendar, FiClock, FiLock, FiCreditCard, FiHome, FiBriefcase, FiX, FiCopy, FiCheck, FiMail, FiAlertCircle, FiDownload
} from 'react-icons/fi';
...
const StatutoryCertificates = () => {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      if (data.success && data.data && data.data.data) {
        ...
        const transformedDocuments = data.data.data.map(doc => ({
          id: doc.id,
          title: doc.name,
          description: doc.description || `${doc.documentType} Certificate`,
          date: doc.year?.toString() || new Date(doc.createdAt).getFullYear().toString(),
          size: getFileSize(doc.documentPath),
  ...
  // Helper function to get file size from path
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      if (data.success && data.data.verified === true) {
        ...
        if (certificateToView) {
          setSelectedCertData({ ...certificateToView, userData: userInfo });
          setShowModal(true);
          setCertificateToView(null);
        }
        
        alert('✅ OTP Verified Successfully! You can now view certificates.');
      } else {
  ...
  const handleViewCertificate = (cert) => {
    ...
    if (storedUser) {
      ...
      if (hoursDiff < 24) {
        setSelectedCertData({ ...cert, userData: user });
        setShowModal(true);
        setCertificateToView(null);
      } else {
        localStorage.removeItem('msrs_verified_user');
      ...
      <section className="py-24 bg-[#F7F9F5]">
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="text-center mb-12" data-aos="fade-up">
            ...
            <p className="text-[#4A5C46] text-sm max-w-2xl mx-auto">
              View and download official documents from our registry
            </p>
            {isVerified && (
              <div className="mt-4 inline-flex items-center gap-2 bg-green-100 text-green-700 px-3 py-1 text-xs rounded-full">
                <FiCheckCircle size={12} />
                <span>✓ Verified User: {userData?.name}</span>
                <button 
          ...
          {loadingDocuments ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-500">No documents found in the database.</p>
              <p className="text-xs text-gray-400 mt-2">Please add documents using the admin panel.</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-[#2C3E2B] text-sm truncate">{cert.title}</h4>
                        <p className="text-[9px] text-gray-500 truncate">{cert.description}</p>
                    ...
                    <button 
                      ...
                      className="px-3 py-1.5 bg-[#667A62] text-white text-[10px] font-semibold hover:bg-[#4A5C46] transition-all flex items-center gap-1 flex-shrink-0 ml-2"
                    >
                      <FiEye size={12} /> View
                    </button>
                  </div>
            ...
            <div className="bg-[#2C3E2B] p-4 text-white sticky top-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-serif text-lg font-bold">Certificate Document</h3>
              ...
              <div className="certificate-watermark disable-select relative bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-[#667A62]">
                ...
                <div className="p-6 text-center">
                  ...
                  <div className="bg-white p-3 mb-4 shadow-sm border border-gray-200">
                    ...
                    {selectedCertData?.year && (
                      <p className="text-[9px] text-gray-400 mt-1">Year: {selectedCertData.year}</p>
                    )}
                  </div>
                  
                  <div className="my-4 border border-gray-200 overflow-hidden bg-gray-100 min-h-[200px] flex items-center justify-center p-4">
                    {selectedCertData?.documentPath ? (
                      <div className="text-center">
                        ...
                        <a 
                          href={getDocumentUrl(selectedCertData.documentPath)} 
                          target="_blank" 
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 px-4 py-2 bg-[#667A62] text-white text-xs rounded hover:bg-[#4A5C46] transition"
                        >
                          <FiDownload size={12} /> Download Document
                        </a>
                      </div>
                    ) : (
                      <div className="text-center">
                        <FaRegFileAlt className="text-6xl text-gray-400 mx-auto mb-3" />
                        <p className="text-xs text-gray-500">No document file available</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[10px] text-left mt-4 p-3 bg-gray-50">
                    <div>
                      ...
                      <p className="font-semibold text-[9px] break-all">{selectedCertData?.id?.substring(0, 8) || 'N/A'}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Date of Issue:</span>
                      <p className="font-semibold text-[9px]">{selectedCertData?.date || new Date().toLocaleDateString()}</p>
                    </div>
                    <div>
                      <span className="text-gray-500">Location:</span>
                      <p className="font-semibold text-[9px]">
                        {selectedCertData.userData?.country === 'India' 
                          ? `${selectedCertData.userData?.state || 'N/A'}, ${selectedCertData.userData?.district || 'N/A'}, India`
                          : selectedCertData.userData?.country || 'India'}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500">Email:</span>
                      <p className="font-semibold text-[9px] break-all">{selectedCertData.userData?.email || userData?.email}</p>
                    </div>
                  </div>
                  
                  <div className="border-t border-gray-200 pt-3 mt-4">
                    <p className="text-center text-[8px] text-gray-400">
                      This is a digitally verified certificate from MSRS Foundation.
                    </p>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-5 p-3 bg-blue-50 rounded-lg border border-blue-100">
                    ...
                    <p className="text-[10px] text-blue-800 leading-relaxed">
                      Government verified documents require identity confirmation. Your information is secure and encrypted.
                    </p>
                  </div>
